...
Workbook::XF::XF() : Record(),
	fontRecordIndex_(0), formatRecordIndex_(0), protectionType_((unsigned short)0xFFF5),
...
Workbook::Style::Style() : Record(),
	XFRecordIndex_((unsigned short)0x8000), identifier_(0), level_((unsigned char)0xFF)
{
	code_ = CODE::STYLE;
...
#ifdef _DEBUG
				case 0:
				case (unsigned short)0xcdcd:
				case (unsigned short)0xfdfd:
//					assert(0);//@@
					break;
#endif
...
Worksheet::CellTable::RowBlock::CellBlock::Formula::Formula() : Record(),
	rowIndex_(0), colIndex_(0), XFRecordIndex_(0), options_(0), unused_(0), type_((USHORT)-1)
{
	code_ = (unsigned)CODE::FORMULA;
	// MF was 18/22 instead of 20/24
	dataSize_ = 20;
	recordSize_ = 24;
}	
...
Worksheet::CellTable::RowBlock::Row::Row() : Record(),
	rowIndex_(0),
	firstCellColIndex_(0),
	lastCellColIndexPlusOne_(0),
	height_(255),		// row height in twips, 255 => 12.75pt
	unused1_(0), unused2_(0),
	options_(0x100/*MF: documentation says "Always 1" for the 0x100 bit*/)
{
	code_ = CODE::ROW;
	dataSize_ = 16;
	recordSize_ = 20;
...
ULONG Worksheet::CellTable::RowBlock::RecordSize()
{
	return DataSize();
}
...
void BasicExcel::UpdateYExcelWorksheet()
{
	...
	for(int i = 0; i < maxWorksheets; ++i) {
		YExcel::Worksheet &sheet = (worksheets_[i]);
		yesheets_.push_back(new BasicExcelWorksheet(this, i));

		for(size_t j = 0; j < sheet.colinfos_.colinfo_.size(); ++j)
		{
			Worksheet::ColInfo &co = (sheet.colinfos_.colinfo_[j]);

			// we're ignoring "co.options" here...
			for(USHORT k = co.firstColumnIndex_; k < co.lastColumnIndex_; k++)
				yesheets_[i]->SetColWidth(k, co.columnWidth_);
		}
	}
}
...
void BasicExcel::UpdateWorksheets()
{
	...
	for(int s=0; s<maxWorksheets; ++s) {
		...
		//Modify ColInfo
		rawSheet.colinfos_.colinfo_.clear();
		USHORT defColSz = sheet.GetDefaultColWidth();
		for (USHORT k = 0; k < maxCols; k++)
		{
			USHORT thisCol = sheet.GetColWidth(k);
			if (thisCol != defColSz)
			{
				USHORT j;
				for(j = k; j < maxCols && sheet.GetColWidth(j) == thisCol; )
					j++;
				Worksheet::ColInfo col;
				col.columnWidth_ = thisCol;
				col.firstColumnIndex_ = k;
				col.lastColumnIndex_ = j;  // actually last plus 1...
				rawSheet.colinfos_.colinfo_.push_back(col);
		...
		for(USHORT r = 0, curRowBlock = 0; r < maxRows; ++r) {
			if (r % 32 == 0) {
...
//				if (cellType != BasicExcelCell::UNDEFINED)	// Current cell contains some data
				// Write cell content, even if blank in order to keep format
				{
					...
					if (newRow) {
						// If there's a rowinfo record for this row, apply the height there.
						USHORT rowHeight = sheet.GetRowHeight(r);
						...
						rRowBlock.rows_.back().height_ = rowHeight;
					...
					switch(cellType) {
						case BasicExcelCell::INT:
						{
							...
							if (cl > c+1) {
								...
								pCell->_union.mulrk_->XFRK_.resize((cl-(int)c));
...
BasicExcelWorksheet::BasicExcelWorksheet(BasicExcel* excel, int sheetIndex) :
	excel_(excel), sheetIndex_(sheetIndex), defaultColWidth(2550), defaultRowHeight(255)
{
	UpdateCells();
}
...
{
	rowInfo_[colindex] = colwidth;
}

// Get the colwidth for the given col
USHORT BasicExcelWorksheet::GetColWidth(const int colindex) const
{
	if (rowInfo_.find(colindex) == rowInfo_.end())
		return GetDefaultColWidth();
	else
		return rowInfo_.at(colindex);
}

void BasicExcelWorksheet::SetRowHeight(const int rowindex, const USHORT height)
{
	rowInfo_[rowindex] = height;
}

// Get the rowwidth for the given row
USHORT BasicExcelWorksheet::GetRowHeight(const int rowindex) const
{
	if (rowInfo_.find(rowindex) == rowInfo_.end())
		return GetDefaultRowHeight();
	else
		return rowInfo_.at(rowindex);
}

void BasicExcelWorksheet::SetDefaultRowHeight(const USHORT height)
{
	defaultRowHeight = height;
}

USHORT BasicExcelWorksheet::GetDefaultRowHeight() const
{
	return defaultRowHeight;
}

void BasicExcelWorksheet::SetDefaultColWidth(const USHORT width)
{
	defaultColWidth = width;
}

USHORT BasicExcelWorksheet::GetDefaultColWidth() const
{
