...
struct CODE
{
	enum {
			...
			DEFROWHEIGHT = 0x0025, 	//Default row height for rows that do not have a ROW record
...
public: // Cell functions
	...
	USHORT GetColWidth(const int colindex) const;

	void SetRowHeight(const int rowindex, const USHORT height);
	USHORT GetRowHeight(const int rowindex) const;

	void SetDefaultColWidth(const USHORT colwidth);
	USHORT GetDefaultColWidth() const;

	void SetDefaultRowHeight(const USHORT height);
	USHORT GetDefaultRowHeight() const;
...
private:
	...
	USHORT defaultColWidth;
	USHORT defaultRowHeight;
	vector<vector<BasicExcelCell> > cells_;		///< Cells matrix.
	map<int, USHORT> colInfo_;			/// used to record row info per col
	map<int, USHORT> rowInfo_;			/// used to record row info per row
};
...
public:
	...
	short GetXFormatIdx() const					{return _xf_idx;}	//MF
	void SetXFormatIdx(short xf_idx)			{ _xf_idx = xf_idx;} //MF
